...
Now, let's introduce some more functions that are helpful for data cleaning that we haven't seen yet. 
...
You may need data from multiple data sets to be in one data set. We do this with `join()` functions. The most common is `left_join()`, but you may also see `right_join()`, `inner_join()`, and `full_join()`.

A `left_join()` adds variables from a second dataset to your first. This is what I almost exclusively use. 
...
Sometimes you'll want to pivot your data longer. For example, if you want to have month be a variable in a regression (where you control for the month), but instead there are 12 columns for the month. You can pivot those columns into one column called `month`.
