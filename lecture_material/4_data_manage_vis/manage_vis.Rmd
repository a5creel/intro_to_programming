...
*Pull it into the `raw_data` file we created while going through file structure. Create a script called `0_data_clean.R` in the `scripts` folder*
...
# Look aat the dataset so we know what variables we have etc
# View(myData_og)
...
- We'll do 6 chats together to get a feel for it 
...
# -------------------------------------------------------------------------
myData %>% # pipe data 
  mutate(cyl = as.factor(cyl)) %>% # change a variable to be categorical (factor())
...
# -------------------------------------------------------------------------
myData %>% # example of us piping in the data (could have this in the gglot() function)
  ggplot(aes(x = hwy)) +
    # the geom_...() functions can have different inputs to change their looks
...
# -------------------------------------------------------------------------
# Multiple plots -- when you wanna see the same graph for multiple classes 
# -------------------------------------------------------------------------
...
# ggsave("results/facet.png", # file path and name
#        plot = myFacet, # plot you want to save
#        width = 10, # width of plot (inches)
#        height = 8, # height of plot (inches)
#        dpi = 300) # dots per square inch)

# -------------------------------------------------------------------------
# Line chart -- when you wanna see how something changed through time: geom_line()
# -------------------------------------------------------------------------

# we're going to use the economics data set, which is in the ggplot2 package
myLine <- economics %>% 
...
#AC: copy and past this to go save another plot from up above
...
- We couldn't put this in a presentation, it's not easy enough to read
...
Let's take one of our charts and make it polished following a few basic best practices. Data visualization is an art all on it's own, and it's worth taking advantage of other resources in data visualization. 

**Best Practices**

1) Choose the right graph: let your research question guide this
2) Clear labels and titles 
...
## Facet chart
Consider if our research questions gas milage relative to engine size, compared across our companies. 


```{r}
# -------------------------------------------------------------------------
# Working with the facet plot already 
# -------------------------------------------------------------------------
...
myFacet <- ggplot(myData, aes(x = displ, y = hwy)) +
  ...
  # clean up our title
  labs(title = "Engine Size vs Highway MPG by Company", 
       x = "Engine Displacement (litres)",
       y = "Highway Miles per Gallon") +
  
  # choose the right simple theme that are prepackaged in ggplot
  # theme_minimal() +
  theme_bw() + 
...
ggsave("results/facet.png",
       plot = myFacet,
       width = 10, 
       height = 4, # change this so isn't so weirdly shaped
       dpi = 300) 
...
# colors I use: darkred, darkblue, darkgreen, darkorange, 
...
ggsave("results/bar.jpeg",
       plot = myBar,
       width = 10, 
       height = 8, # change this so isn't so weirdly shaped
       dpi = 300) 
